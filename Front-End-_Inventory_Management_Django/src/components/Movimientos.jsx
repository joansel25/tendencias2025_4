// Movimientos.jsx - VERSIÓN COMPLETA CORREGIDA
...
export default function Movimientos() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Intentar cargar proveedores solo si tenemos permisos
      try {
        const provRes = await api.get("/farmacia/proveedores/");
        setProveedores(provRes.data);
      } catch (provError) {
        console.warn(
          "No se pudieron cargar proveedores (posible falta de permisos,)",provError
        );
        setProveedores([]);
        setErrorCarga(
          "Algunos datos pueden estar incompletos debido a permisos limitados."
        );
      }
    } catch (error) {
      console.error("Error cargando datos:", error);
      setErrorCarga("Error al cargar los datos. Verifica tus permisos.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Obtener ID del empleado responsable
      ...
      const movimientoData = {
        ...
        id_proveedor: formData.id_proveedor
          ? parseInt(formData.id_proveedor)
          : null,
  ...
  const descargarPDF = async (movimientoId = null) => {
    try {
      ...
      link.setAttribute(
        "download",
        movimientoId
          ? `movimiento_${movimientoId}.pdf`
          : "todos_movimientos.pdf"
      );
  ...
  // AQUÍ VA EL RETURN COMPLETO CON LA SECCIÓN QUE PREGUNTAS
  return (
    <div
    ...
    >
      <div className="container py-4">
        {/* AQUÍ VA LA SECCIÓN DEL ALERT QUE PREGUNTAS - justo después del container */}
        {errorCarga && (
          <div className="alert alert-warning">
            <strong>Advertencia:</strong> {errorCarga}
          </div>
        )}
                  ...
                  <tbody>
                    {movimientosFiltrados.map((movimiento) => (
                      <tr key={movimiento.id}>
                        ...
                        <td>{movimiento.responsable_nombre || "Sistema"}</td>
                        <td>
